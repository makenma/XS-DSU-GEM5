...
当前 `HnfSLCSF` 是 `HnfCoherencyController` 按值持有的**普通 C++ 对象**，所有接口都是**组合逻辑（0 cycle）同步返回**：
...
现有 11 个用例都是**同步语义**测试（直接调 `lookup()`/`commitRead()`/...）。流水线化后这些方法不再存在。两种处理方式：
...
| 阶段 | 内容 | 验证 | 风险 |
| --- | --- | --- | --- |
| **1** | 抽 `HnfSLCSFBackend`，把现有 `lookup/commitRead/writeLine/...` 整体搬入；`HnfSLCSF` 改为持有 backend + 队列骨架，`tick()` 为空 | 现有 11 个测试改测 backend，全绿 | 低；纯重构 |
