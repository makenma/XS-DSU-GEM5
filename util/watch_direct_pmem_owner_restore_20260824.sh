...
# Restore only after the seventh cold checkpoint passes every semantic gate,
...
cold_out=${repo}/m5out/linux-16core-128mb-direct-pmem-owner-cold-v7-detached-20260824
cold_cpt_parent=${cold_out}/checkpoints
cold_audit=${cold_out}/audit
gate_log=${cold_audit}/restore_gate.log
gate_lock=${cold_audit}/restore_gate.v9.lock
restore_out=${repo}/m5out/linux-16core-128mb-direct-pmem-owner-restore-v9-detached-20260824
...
note "restore gate v9 started after exact frame-semantics correction; v8 was invalidated solely by sandbox socket denial at tick 0; waiting for exact 15/15 direct-pmem-owner audit; PMP storm threshold=${pmp_fault_storm_threshold}; restored checkpoint output=${restore_cpt_parent}"
...
for status_name in gzip chi_audit stack_audit classic_route; do
...
grep -Fq 'saved_s0_chain' "${cold_audit}/stack_audit.txt" ||
    fail "cold stack audit does not identify the corrected frame-chain invariant"
grep -q 'Exiting @ tick .* because checkpoint' "${cold_out}/simout" ||
    fail "cold run lacks normal checkpoint exit marker"
sha256sum -c "${cold_audit}/compressed.sha256" \
    > "${cold_audit}/restore_v9_checkpoint_hash_check.txt" 2>&1 ||
    fail "cold checkpoint hashes changed after audit"
sha256sum -c "${cold_audit}/acceptance_inputs.sha256" \
    > "${cold_audit}/restore_v9_acceptance_input_hash_check.txt" 2>&1 ||
    fail "cold acceptance program or payload hash changed after audit"
...
printf '%s\n' \
    'echo FINAL_DIRECT_PMEM_OWNER_RESTORE_TERMINAL_ACCEPTED_20260824' \
    "printf 'FINAL_DIRECT_PMEM_OWNER_RESTORE_UNAME='; /bin/busybox uname -a" \
    "printf 'FINAL_DIRECT_PMEM_OWNER_RESTORE_CPU_COUNT='; /bin/busybox grep -c '^processor' /proc/cpuinfo" \
    "printf 'FINAL_DIRECT_PMEM_OWNER_RESTORE_CPU_ONLINE='; /bin/busybox cat /sys/devices/system/cpu/online" \
    "printf 'FINAL_DIRECT_PMEM_OWNER_RESTORE_UPTIME='; /bin/busybox cat /proc/uptime" \
    "printf 'FINAL_DIRECT_PMEM_OWNER_RESTORE_SHELL_PID='; echo \$\$" \
    "printf 'FINAL_DIRECT_PMEM_OWNER_RESTORE_HELPER_SHA256='; /bin/busybox sha256sum /tmp/chi_guest_checkpoint_repo | /bin/busybox cut -d' ' -f1" \
    'echo FINAL_DIRECT_PMEM_OWNER_RESTORE_COMMANDS_DONE_20260824' \
    '/bin/busybox sync' \
    'echo FINAL_DIRECT_PMEM_OWNER_RESTORE_RECHECKPOINT_REQUEST_20260824' \
...
grep -Fxq 'FINAL_DIRECT_PMEM_OWNER_RESTORE_TERMINAL_ACCEPTED_20260824' \
    "${normalized}" || fail "missing executed restore marker"
grep -Eq '^FINAL_DIRECT_PMEM_OWNER_RESTORE_UNAME=Linux .* riscv64 GNU/Linux$' \
    "${normalized}" || fail "missing real restored uname"
grep -Fxq 'FINAL_DIRECT_PMEM_OWNER_RESTORE_CPU_COUNT=16' "${normalized}" ||
    fail "restored guest did not report 16 CPUs"
grep -Fxq 'FINAL_DIRECT_PMEM_OWNER_RESTORE_CPU_ONLINE=0-15' "${normalized}" ||
    fail "restored guest online mask mismatch"
grep -Eq '^FINAL_DIRECT_PMEM_OWNER_RESTORE_UPTIME=[0-9]+\.[0-9]+ [0-9]+\.[0-9]+$' \
    "${normalized}" || fail "missing real restored uptime"
grep -Fxq "FINAL_DIRECT_PMEM_OWNER_RESTORE_HELPER_SHA256=${expected_helper_sha}" \
    "${normalized}" || fail "restored helper SHA256 mismatch"
grep -Fxq 'FINAL_DIRECT_PMEM_OWNER_RESTORE_COMMANDS_DONE_20260824' \
    "${normalized}" || fail "restore command sequence incomplete"

# The immutable cold checkpoint parent remains the restore input. Any new
# checkpoint produced by the restored guest must live under restore_out.
...
if ! grep -Eq '[1-9][0-9]* direct-pmem, 0 routed-nonmem' \
        "${restore_out}/simerr"; then
    printf 'FAIL no nonzero direct-pmem checkpoint writeback\n' \
        > "${restore_audit}/classic_route.status"
    fail "post-restore checkpoint lacks a nonzero direct-pmem writeback"
fi
if grep -Eq '[1-9][0-9]* routed-nonmem' "${restore_out}/simerr"; then
    printf 'FAIL nonzero routed-nonmem checkpoint writeback\n' \
        > "${restore_audit}/classic_route.status"
    fail "post-restore checkpoint writeback traversed a non-memory route"
fi
printf 'PASS\n' > "${restore_audit}/classic_route.status"
grep -E 'checkpoint phase .*direct-pmem.*routed-nonmem' \
    "${restore_out}/simerr" > "${restore_audit}/classic_route.log"
...
raw_dir=/tmp/direct-pmem-owner-restore-audit-v9-20260824
...
sha256sum -c "${cold_audit}/acceptance_inputs.sha256" \
    > "${restore_audit}/acceptance_input_hash_check.txt" 2>&1 ||
    fail "acceptance program or payload changed before post-restore stack audit"
...
note "PASS: restore, terminal commands, direct-pmem route, re-checkpoint, CHI, and 15/15"
