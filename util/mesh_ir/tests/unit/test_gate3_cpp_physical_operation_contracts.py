...
ACCEPTED_INPUTS = tuple(
    (f"baseline_{case.case_id}", case.arch, case.baseline) for case in CASES
) + tuple(
...
POSITIVE_CASES = tuple(
    ...
    for semantic_sha256 in dict.fromkeys(case[2].semantic_sha256 for case in ACCEPTED_INPUTS)
)


@pytest.mark.parametrize("case", POSITIVE_CASES, ids=lambda case: case[0])
def test_cpp_complete_physical_operation_baselines_are_accepted(case, driver, tmp_path):
    _, arch, program = case
...
@pytest.mark.parametrize(
    "case",
    tuple(case for case in CASES if case.expected_code is not None),
    ids=lambda case: case.case_id,
)
def test_cpp_complete_physical_operation_cases_have_exact_diagnostics(case, driver, tmp_path):
