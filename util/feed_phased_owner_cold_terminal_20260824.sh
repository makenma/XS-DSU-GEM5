...
outdir=${repo}/m5out/linux-16core-128mb-phased-owner-cold-detached-20260824
...
printf '%s\n' \
    ...
    'echo PHASED_OWNER_COLD_SHELL_20260824' \
    '/bin/busybox mkdir -p /proc /sys' \
    '/bin/busybox mount -t proc proc /proc || true' \
    '/bin/busybox mount -t sysfs sysfs /sys || true' \
    'echo PHASED_OWNER_PROC_SYS_READY_20260824' \
    '/bin/busybox uname -a' \
    "printf 'PHASED_OWNER_CPU_COUNT='; /bin/busybox grep -c '^processor' /proc/cpuinfo" \
    "printf 'PHASED_OWNER_CPU_ONLINE='; /bin/busybox cat /sys/devices/system/cpu/online" \
    "printf 'PHASED_OWNER_UPTIME='; /bin/busybox cat /proc/uptime" \
    "printf 'PHASED_OWNER_SHELL_PID='; /bin/busybox sh -c 'echo \$\$'" \
    '/bin/busybox sha256sum /tmp/chi_guest_checkpoint_repo' \
    '/bin/busybox sync' \
    'echo PHASED_OWNER_TRIGGER_CHECKPOINT_20260824' \
