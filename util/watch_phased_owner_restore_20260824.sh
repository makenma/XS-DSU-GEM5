...
# Restore only after the fourth cold checkpoint passes every semantic gate,
...
cold_out=${repo}/m5out/linux-16core-128mb-phased-owner-cold-detached-20260824
cold_cpt_parent=${cold_out}/checkpoints
cold_audit=${cold_out}/audit
gate_log=${cold_audit}/restore_gate.log
gate_lock=${cold_audit}/restore_gate.v4.lock
restore_out=${repo}/m5out/linux-16core-128mb-phased-owner-restore-detached-20260824
...
note "restore gate v4 started; waiting for exact 15/15 phased-owner audit; PMP storm threshold=${pmp_fault_storm_threshold}; restored checkpoint output=${restore_cpt_parent}"
...
printf '%s\n' \
    'echo FINAL_PHASED_RESTORE_TERMINAL_ACCEPTED_20260824' \
    "printf 'FINAL_PHASED_RESTORE_UNAME='; /bin/busybox uname -a" \
    "printf 'FINAL_PHASED_RESTORE_CPU_COUNT='; /bin/busybox grep -c '^processor' /proc/cpuinfo" \
    "printf 'FINAL_PHASED_RESTORE_CPU_ONLINE='; /bin/busybox cat /sys/devices/system/cpu/online" \
    "printf 'FINAL_PHASED_RESTORE_UPTIME='; /bin/busybox cat /proc/uptime" \
    "printf 'FINAL_PHASED_RESTORE_SHELL_PID='; echo \$\$" \
    "printf 'FINAL_PHASED_RESTORE_HELPER_SHA256='; /bin/busybox sha256sum /tmp/chi_guest_checkpoint_repo | /bin/busybox cut -d' ' -f1" \
    'echo FINAL_PHASED_RESTORE_COMMANDS_DONE_20260824' \
    '/bin/busybox sync' \
    'echo FINAL_PHASED_RESTORE_RECHECKPOINT_REQUEST_20260824' \
...
grep -Fxq 'FINAL_PHASED_RESTORE_TERMINAL_ACCEPTED_20260824' \
    "${normalized}" || fail "missing executed restore marker"
grep -Eq '^FINAL_PHASED_RESTORE_UNAME=Linux .* riscv64 GNU/Linux$' \
    "${normalized}" || fail "missing real restored uname"
grep -Fxq 'FINAL_PHASED_RESTORE_CPU_COUNT=16' "${normalized}" ||
    fail "restored guest did not report 16 CPUs"
grep -Fxq 'FINAL_PHASED_RESTORE_CPU_ONLINE=0-15' "${normalized}" ||
    fail "restored guest online mask mismatch"
grep -Eq '^FINAL_PHASED_RESTORE_UPTIME=[0-9]+\.[0-9]+ [0-9]+\.[0-9]+$' \
    "${normalized}" || fail "missing real restored uptime"
grep -Fxq "FINAL_PHASED_RESTORE_HELPER_SHA256=${expected_helper_sha}" \
    "${normalized}" || fail "restored helper SHA256 mismatch"
grep -Fxq 'FINAL_PHASED_RESTORE_COMMANDS_DONE_20260824' \
...
raw_dir=/tmp/phased-owner-restore-audit-20260824
