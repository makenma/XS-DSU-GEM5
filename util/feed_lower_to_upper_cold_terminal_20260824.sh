...
printf '%s\n' \
    ...
    'echo LOWER_TO_UPPER_COLD_SHELL_20260824' \
    '/bin/busybox mkdir -p /proc /sys' \
    '/bin/busybox mount -t proc proc /proc || true' \
    '/bin/busybox mount -t sysfs sysfs /sys || true' \
    'echo LOWER_TO_UPPER_PROC_SYS_READY_20260824' \
    '/bin/busybox uname -a' \
    "printf 'LOWER_TO_UPPER_CPU_COUNT='; /bin/busybox grep -c '^processor' /proc/cpuinfo" \
    "printf 'LOWER_TO_UPPER_CPU_ONLINE='; /bin/busybox cat /sys/devices/system/cpu/online" \
    "printf 'LOWER_TO_UPPER_UPTIME='; /bin/busybox cat /proc/uptime" \
    "printf 'LOWER_TO_UPPER_SHELL_PID='; /bin/busybox sh -c 'echo \$\$'" \
    '/bin/busybox sha256sum /tmp/chi_guest_checkpoint_repo' \
    '/bin/busybox sync' \
    'echo LOWER_TO_UPPER_TRIGGER_CHECKPOINT_20260824' \
